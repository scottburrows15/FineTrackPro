...
export default function ManualPaymentModal({ isOpen, onClose, fine }: ManualPaymentModalProps) {
  ...
  const recordPayment = useMutation({
    ...
    onSuccess: () => {
      ...
      // Comprehensive cache invalidation to ensure monthly collection updates immediately
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="w-full max-w-[95vw] sm:max-w-md p-4 sm:p-6 max-h-[90vh] overflow-y-auto bg-white dark:bg-card border-border" aria-describedby="payment-description">
        <DialogHeader>
